...
<style type="text/css">
  .click2pay-pix-open-browser-container {
    max-width: 300px;
    margin: 20px auto;
    text-align: center;
  }

  .click2pay-pix-open-browser {
  ...
  .click2pay-pix-copy-paste {
    max-width: 930px;
    padding: 0 0 20px 20px;
    box-sizing: border-box;
  }

  .click2pay-pix-copy-paste .click2pay-pix-copy-title {
    margin: 0 0 15px 0;
    padding: 0;
    font-size: 18px;
    font-weight: 600;
  ...
  .click2pay-pix-copy-paste .click2pay-pix-copy-title span {
    font-size: 13px;
    background: #1e262c;
    color: #fff;
    font-weight: normal;
  ...
  .click2pay-pix-copy-paste .click2pay-pix-copy-title span:hover {
    background: #4e7f30;
  }

  .click2pay-pix-copy-paste .click2pay-pix-url {
    background: #efefef;
    text-align: center;
    padding: 15px 20px;
    font-size: 13px;
  ...
  .click2pay-pix-container {
    display: flex;
    align-items: center;
    max-width: 930px;
  }

  .click2pay-pix-container .qr-code,
  .click2pay-pix-container img {
    width: 350px;
    min-width: 350px;
  }
  ...
  .click2pay-pix-container .click2pay-pix-instructions {
    flex: 1;
  }

  .click2pay-pix-container .click2pay-pix-instructions ul {
    padding: 0;
    margin: 0 0 0 10px;
    list-style: none;
  }

  .click2pay-pix-container .click2pay-pix-instructions ul li {
    margin: 15px 0 20px;
    display: flex;
    align-items: center;
  ...
  .click2pay-pix-container .click2pay-pix-instructions svg {
    min-width: 45px;
    width: 45px;
    height: 45px;
    margin-right: 20px;
  ...
  .click2pay-pix-container .click2pay-pix-instructions .click2pay-pix-mobile-only {
    display: none;
  }
  ...
  @media only screen and (max-width: 650px){
    ...
    .click2pay-pix-container .click2pay-pix-instructions .click2pay-pix-desktop-only {
      display: none;
    }
    .click2pay-pix-container .click2pay-pix-instructions .click2pay-pix-mobile-only {
      display: inline;
...
<section id="<?php echo esc_attr( $id ); ?>-thankyou">
	<h3 class="click2pay-pix-title"><?php echo __( 'Aguardando sua transferência via Pix', 'click2pay-for-woocommerce' ); ?></h3>
	<?php if ( $instructions ) { ?>
		<div class="instruction"><?php echo $instructions; ?></div>
	<?php } ?>

  <?php if ( isset( $order ) && $is_email ) { ?>
    <div class="click2pay-pix-open-browser-container">
      <a href="<?php echo $pix_details_page; ?>" class="click2pay-pix-open-browser">
        <?php echo __( 'Ver QR Code no navegador', 'click2pay-for-woocommerce' ); ?>
      </a>
    </div>
  <?php } ?>

	<div class="click2pay-pix-copy-paste">
		<h3 class="click2pay-pix-copy-title"><?php echo __( 'Pix Copia e cola', 'click2pay-for-woocommerce' ); ?>
      <?php if ( ! $is_email ) {
        echo '<span class="pix-copy pix-copy-button" data-clipboard-target="#click2pay-pix-payload">' . __( 'Clique para copiar', 'click2pay-for-woocommerce' ) . '</span>';
      } ?>
    </h3>
		<textarea style="width: 100%;" id="click2pay-pix-payload" readonly data-clipboard-target="#click2pay-pix-payload" class="pix-copy click2pay-pix-url"><?php echo $payload; ?></textarea>
	</div>

	<div class="click2pay-pix-container">
    <?php if ( ! $is_email ) { ?>
      <div class="qr-code">
        <img
          src="<?php echo $pix_image; ?>"
          alt="<?php echo __( 'QR Code para pagamento', 'click2pay-for-woocommerce' ); ?>"
        />
      </div>
    <?php } ?>

		<div class="click2pay-pix-instructions">
			<ul>
				<li>
				...
				<div><?php echo __( 'Abra o app do seu banco ou instituição financeira e <strong>entre no ambiente Pix</strong>.', 'click2pay-for-woocommerce' ); ?></div></li>
				...
				<li>
					...
					<div>
            <?php if ( $is_email ) {
              echo __( 'Escolha a opção <strong>Pix Copia e Cola</strong> e insira o texto acima<strong></strong>.', 'click2pay-for-woocommerce' );
            } else {
              echo __( 'Escolha a opção <strong> Pagar com QR Code</strong> e escaneie o código <span class="click2pay-pix-mobile-only"> abaixo</span><span class="click2pay-pix-desktop-only"> ao lado</span> ou utilize o recurso <strong>Pix Copia e Cola</strong>.', 'click2pay-for-woocommerce' );
            } ?>
          </div>
				</li>
				...
				<li>
					...
					<div>
            <?php echo __( 'Confirme as informações e <strong>finalize o pagamento</strong>. Pode demorar alguns minutos até que o pagamento seja confirmado. Iremos avisar você!', 'click2pay-for-woocommerce' ); ?>
          </div>
				</li>
			</ul>
		</div>
